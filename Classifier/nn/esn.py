...
class ESN(nn.Module):
    ...
    State Network is based on the paper:
    ...
    :param output_size: (int) The number of expected features in the output y.
    :param num_layers: (int, optional) Number of recurrent layers. Default: 1.
    :param nonlinearity: (str, optional) The non-linearity to use ['tanh'|'relu'|'id'].
        Default: 'tanh'.
    :param batch_first: (bool, optional) If True, then the input and output tensors are provided
        as (batch, seq, feature). Default: False.
    :param leaking_rate: (float, optional) Leaking rate of reservoir's neurons. Default: 1.
    :param spectral_radius: (float, optional) Desired spectral radius of recurrent weight matrix.
        Default: 0.9.
    :param w_ih_scale: (float or torch.Tensor, optional) Scale factor for first layer's input weights (w_ih_l0). It
        can be a number or a tensor of size '1 + input_size' and first element
        is the bias' scale factor. Default: 1.
    :param lambda_reg: (float, optional) Ridge regression's shrinkage parameter. Default: 1.
    :param density: (float, optional) Recurrent weight matrix's density. Default: 1.
    :param w_io: (bool, optional) If True, then the network uses trainable input-to-output
        connections. Default: False.
    :param readout_training: (str, optional) Readout's training algorithm ['gd'|'svd'|'cholesky'|'inv'].
    ...
    :param output_steps: (str, optional) Defines how the reservoir's output will be used by ridge
        regression method ['all', 'mean', 'last'].
        If 'all', the entire reservoir output matrix will be used.
        If 'mean', the mean of the reservoir output matrix along the timesteps
    ...
    Inputs:
        input (torch.Tensor): tensor containing the features of the input sequence of shape
            (seq_len, batch, input_size). The input can also be a packed variable length
            sequence. See `torch.nn.utils.rnn.pack_padded_sequence`.
        washout (torch.Tensor): number of initial timesteps during which output of the
            reservoir is not forwarded to the readout. One value per batch's sample.
        h_0 (torch.Tensor, optional): tensor containing the initial reservoir's hidden state
            for each element in the batch of shape (num_layers, batch, hidden_size).
            Defaults to zero if not provided.
        target (torch.Tensor): tensor containing the features of the batch's target sequences
            rolled out along one axis, minus the washouts and the padded values of shape
            (seq_len * batch - washout * batch, output_size). It is only needed
            for readout's training in offline mode. Use `prepare_target` to
            compute it.

    Outputs:
        tuple:
            - output (torch.Tensor): tensor containing the output features (h_k) from the readout,
              for each k, of shape (seq_len, batch, hidden_size).
            - h_n (torch.Tensor): tensor containing the reservoir's hidden state for k=seq_len of shape
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=1,
        ...
        self.reservoir = Reservoir(mode, input_size, hidden_size, num_layers, leaking_rate, spectral_radius,
                                   self.w_ih_scale, density, batch_first=batch_first)
    ...
    def fit(self):
        ...
        if self.readout_training == 'cholesky':
            ...
            I = (self.lambda_reg * torch.eye(self.XTX.size(0))).to(self.XTX.device)
            A = self.XTX + I
